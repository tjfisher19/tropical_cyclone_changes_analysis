...
data_line <- "gray60"
seg_line <- "gray20"
caption_text <- "Source: Advanced Dvorak Technique-Hurricane Satellite Data (ADT-HURSAT)\nhttps://www.ncei.noaa.gov/products/advanced-dvorak-technique-hurricane-satellite"
...
global_basin <- hursat_non_shorty_for_props %>%
  group_by(SEASON) %>%
...
global_key_results_plot <- hursat_key_results |>
...
storm_counts <- hursat_non_shorty_for_props



storm_basin_regimes <- hursat_key_results |>
  dplyr::filter(Method=="Changepoint") |>
  mutate(adj = p.adjust(p.value, method="fdr")) |>
...
ggsave(filename="./plots/result_hursat_basin_hurricanes_findings.png", 
